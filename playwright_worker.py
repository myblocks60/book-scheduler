...
#     try:
#         # Dynamic placeholders injected from frontend
...
#             page,
#             ['input[placeholder="e.g. Hospitality, IT, Healthcare"]', 'xpath=//*[@id="root"]/div/div/div[2]/div[1]/input'],
#             subtopic
#         )

#         await fill_with_fallback(
#             page,
#             ['input[placeholder="e.g. Customer Service Excellence"]', 'xpath=//*[@id="root"]/div/div/div[2]/div[2]/input'],
#             topic
#         )

#         await fill_with_fallback(
#             page,
#             ['textarea[placeholder="Describe what the book should focus on..."]', 'xpath=//*[@id="root"]/div/div/div[2]/div[3]/div/textarea'],
#             final_prompt
#         )

#         await click_with_fallback(
#             page,
#             ["role=button[name='Generate Book']", "text=Generate Book", "button.primary-button"]
#         )

#         await wait_for_generation_complete(page)

#         # Actually click the Download button and save it
#         if await page.locator("text=Download").is_visible() or await page.locator(".success-box").is_visible():
#             try:
#                 async with page.expect_download(timeout=120000) as download_info:
#                     await click_with_fallback(page, ["text=Download", "button:has-text('Download')", "a:has-text('Download')"])
#                 download = await download_info.value
#                 save_path = DOWNLOAD_DIR / f"{career.replace(' ', '_')}.pdf"
#                 await download.save_as(save_path)
#                 logging.info(f"PDF saved successfully: {save_path}")
#             except Exception as d_err:
#                 logging.warning(f"Failed to save PDF download: {d_err}")

#         if connect_rag:
#             try:
#                 logging.info("Attempting to connect to RAG server...")
#                 await page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
                
#                 # Fixed: Use separate selectors instead of a broken comma list
#                 await page.wait_for_selector("button:has-text('Connect to RAG Server')", timeout=15000)
...
#                 await page.wait_for_selector('input[placeholder="e.g. 1559"]', state="visible", timeout=15000)
#                 await fill_with_fallback(page, ['input[placeholder="e.g. 1559"]'], "1559")
#                 await click_with_fallback(page, ["role=button[name='Connect']", "text=Connect"])
#                 await page.wait_for_function("() => !document.querySelector('input[placeholder=\"e.g. 1559\"]')", timeout=20000)
#                 logging.info("RAG Connection successful.")
#             except Exception as rag_err:
#                 logging.warning(f"RAG Connection button not found or already connected: {rag_err}. Proceeding to upload...")

#         # Scroll again before selecting category
#         await page.evaluate("window.scrollTo(0, document.body.scrollHeight)")

#         # More robust selector for the category dropdown
#         select_locator = page.locator("select").first
#         await select_locator.wait_for(state="attached", timeout=60000)
#         await select_locator.wait_for(state="visible", timeout=60000)
#         await page.wait_for_function("""
#         () => {
#             const sel = document.querySelector("select");
#             return sel && sel.options.length > 0 && Array.from(sel.options).some(o => o.text.toLowerCase().includes("career"));
#         }
#         """, timeout=60000)

#         await page.select_option("select", value="Career book")
#         logging.info("Selected Career book category")

#         await click_with_fallback(page, ["role=button[name='📚 Add to RAG']", "text=Add to RAG"])
#         logging.info("Clicked Add to RAG")

#         await page.wait_for_function("""
#         () => {
#             const btn = Array.from(document.querySelectorAll("button")).find(b => b.innerText.includes("Adding") || b.disabled);
#             return !!btn;
#         }
#         """, timeout=10000)
...
#         await page.wait_for_function("""
#         () => {
#             const btn = Array.from(document.querySelectorAll("button")).find(b => b.innerText.includes("Adding"));
